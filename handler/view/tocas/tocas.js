...
window.tocas_modules = [...window.tocas_modules, new Tooltip()]
    ...
    addedNodeMutation = added_node => {
        // 如果這個新追加的 DOM 節點是一個 Toggle 模組，就監聽其點擊事件。
        if (this.isClickAway(added_node)) {
            this.bindEventListener(added_node);
        }
    };
    ...
    // isClickAway
    isClickAway = element => {
        return element.matches("[data-clickaway]");
    };

    // toggle
    toggle = event => {
        if (event.target.closest("[data-clickaway]") === event.target) {
            event.target.close();
        }
    };
    ...
    bindEventListener = element => {
        ...
        element.removeEventListener("mousedown", this.toggle);
        element.addEventListener("mousedown", this.toggle);
    };
}

window.tocas_modules = [...window.tocas_modules, new ClickAway()]
